...
var (
	CtxOrmV2 = new(myGormSQLManagerV2)
)

func init() {
	CtxOrmV2.Once.Do(func() {
		CtxOrmV2.clients = make(map[string]*gorm.DB)
	})
}
 ...
 * @return	错误
 */
func (manager *myGormSQLManagerV2) readOption(name string) (map[string]string, error) {
...
//--> @return 错误
//-- ----------------------------
func (manager *myGormSQLManagerV2) createClient(name string) (*gorm.DB, error) {
	var (
		err   error
		dbIns *gorm.DB
		db    *sql.DB
	...
	if dbIns, err = gorm.Open(sqlGorm.Open(dbConfig["source"]), &gorm.Config{}); err != nil {
		return nil, err
	}
	if db, err = dbIns.DB(); nil != err {
		return nil, err
	}
	// 最大空闲链接
	maxIdleCons := 10
...
//--> @return 错误
//-- ----------------------------
func (manager *myGormSQLManagerV2) GetClient(name string) (*gorm.DB, error) {
	manager.mu.Lock()
	...
	if err == nil {
		if mysqlViper.GetBool("debug") {
			client = client.Model(true).Debug()
		}
	}
	return client, nil
}
...
//--> @return 无
//-- ----------------------------
func (manager *myGormSQLManagerV2) Release(name string) {
	manager.mu.Lock()
...
//--> @return 无
//-- ----------------------------
func (manager *myGormSQLManagerV2) ReleaseAll() {
	manager.mu.Lock()
