...
func (p *Pool) makeRecClient() (err error) {
	...
	p.wgRec.Add(1)
	if connRec4, err = p.makeRecConn4(); nil != err {
		return
	}
	go p.revIpv4(connRec4)

	if !p.option.v6 {
		return
	}
	p.wgRec.Add(1)
	if connRec6, err = p.makeRecConn6(); nil != err {
		return
	}
	go p.revIpv6(connRec6)
	return
}
...
// --> @return
// -- ------------------------------------
func (p *Pool) startSenderV4(connSend packetConn) {
	defer p.option.Recover()
LOOP:
	for {
		select {
...
// --> @return
// -- ------------------------------------
func (p *Pool) startSenderV6(connSend packetConn) {
	defer p.option.Recover()
LOOP:
	for {
		select {
...
func (p *Pool) sendICMP(ps *Ping, conn packetConn) (err error) {
	...
	//if msgBytes, err = ps.Msg.Marshal(nil); err != nil {
	//	return
	//}
	...
	defer p.seqP.Put(b)
	if ps.IsOrderSeq {
		binary.BigEndian.PutUint16(b, uint16(ps.Sequence))
		ps.Data[6],
			ps.Data[7] =
			b[0],
			b[1]
	...
	for {
		if _, err = conn.WriteTo(ps.Data, dst); err != nil {
