...
// --> @return
// -- ------------------------------------
func (p *Pool) revIpv4(conn net.PacketConn) {
...
// --> @return
// -- ------------------------------------
func (p *Pool) revIpv6(conn net.PacketConn) {
	defer p.option.Recover()
	// Start by waiting for 50 µs and increase to a possible maximum of ~ 100 ms.
	expBackoffTime := newExpBackoff(50*time.Microsecond, 11)
	delay := expBackoffTime.Get()
...
func (p *Pool) processPacket(receivedAt time.Time,
	...
	if isEcho, Id, seq, data, err = ParseMessage(proto, bytes); err != nil {
		return fmt.Errorf("error parsing icmp message: %w", err)
	}
	...
	if len(data) < timeSliceLength+trackerLength {
		return fmt.Errorf("insufficient data received; got: %d %v",
			len(data), data)
	}
	if pktUUID = p.getPacketUUID(data); err != nil {
		return err
	}
	...
	pkg.StartTime,
		...
		pkg.ID,
		pkg.uuid =
		bytesToTime(data[:timeSliceLength]),
		receivedAt,
		ip,
		nBytes,
		seq,
