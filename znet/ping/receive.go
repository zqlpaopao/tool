...
func (p *Pool) revIpv4(conn net.PacketConn, id int) {
	...
	for {
		...
		default:
			...
			if err = p.processPacket(pktR, id); nil != err {
				p.packetP.Put(pktR)
...
// --> @return
// -- ------------------------------------
func (p *Pool) processPacket(rev *packet, id int) (err error) {
	var (
		proto   int
		m       *icmp.Message
		ip      string
		pktUUID = UUIDPool.Uuid.Get()
		str     string
	)
	if rev.ipv4 {
		proto = protocolICMP
	...
	case *icmp.Echo:
		...
		if pktUUID, err = p.getPacketUUID(pkt.Data); err != nil {
			return err
		}
		str = pktUUID.String()

		defer UUIDPool.Uuid.Put(pktUUID)
		if _, ok := p.get(str); !ok {
