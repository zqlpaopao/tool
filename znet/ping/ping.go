...
func (p *Pool) getPacketUUID(pkt []byte) (string, error) {
	return src.Bytes2String(pkt[timeSliceLength : timeSliceLength+trackerLength]), nil
}

// getCurrentTrackerUUID grabs the latest tracker UUID.
func (p *Ping) getCurrentTrackerUUID() string {
	return p.Uuid
}
...
func (p *Pool) timeToBytes(t time.Time) []byte {
	nSec := t.UnixNano()
	//b := make([]byte, 8)
	b := p.timeToBytesP.Get()
	for i := uint8(0); i < 8; i++ {
