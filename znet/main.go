...
func main() {
	...
	go Loop()
	http.ListenAndServe("0.0.0.0:6069", nil)
...
func Loop() {
	...
	p := ping.NewPoolWithOptions(
		ping.WithPrepareChLen(12),
		ping.WithResChanLen(12),
		ping.WithPingPoolLen(12),
		ping.WithSendWorker(1),
		ping.WithCallbackWorker(1),
	...
	for k := range mp {
		...
		bd.SetDstAddr(k).SetIpV4().SetResolveIpAddr(&net.IPAddr{IP: ip}).SetSize(60).SetTtl(60).SetUUid(ping.NewUUid().String()).SetPrivileged(true).SetNetwork("icmp")
		rs[k] = bd
	}
	...
	for {
		...
		select {
		case <-t.C:
			fmt.Println("end------------------------", atomic.LoadInt32(&num), time.Now().Format(time.DateTime))
...
var rs = make(map[string]*ping.Ping, 5000)
