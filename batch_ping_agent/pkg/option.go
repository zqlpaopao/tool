...
func newPool(opt *Option) *Pool {
	return &Pool{
		...
		tData:       &pkg.Pool[[]byte]{Data: make(chan *[]byte, opt.sendPool.tData), New: func() *[]byte { b := make([]byte, 24); return &b }},
		body:        &pkg.Pool[icmp.Echo]{Data: make(chan *icmp.Echo, opt.sendPool.body), New: func() *icmp.Echo { return &icmp.Echo{} }},
		msg:         &pkg.Pool[icmp.Message]{Data: make(chan *icmp.Message, opt.sendPool.msg), New: func() *icmp.Message { return &icmp.Message{} }},
		msgBytes:    &pkg.Pool[[]byte]{Data: make(chan *[]byte, opt.sendPool.msgBytes), New: func() *[]byte { return &[]byte{} }},
		uuidEncoded: &pkg.Pool[[]byte]{Data: make(chan *[]byte, opt.sendPool.uuidEncoded), New: func() *[]byte { return &[]byte{} }},
	}
...
func defaultOption() *Option {
	...
	return &Option{
		...
		sendPool: struct {
			...
			tData       int
			uuidEncoded int
		}{body: SendSize, msg: SendSize, msgBytes: SendSize, tData: SendSize, uuidEncoded: SendSize},
	}
}
...
func WithSendTDataSize(tDataSize int) OpFunc {
	return func(option *Option) {
		option.sendPool.tData = tDataSize
	}
}
func WithSendMsgBytesSize(msgBytesSize int) OpFunc {
	return func(option *Option) {
		option.sendPool.msgBytes = msgBytesSize
	}
}
func WithSendMsgSize(msgSize int) OpFunc {
	return func(option *Option) {
		option.sendPool.msg = msgSize
	}
}
func WithSendBodySize(bodySize int) OpFunc {
	return func(option *Option) {
		option.sendPool.body = bodySize
	}
}
func WithSendUuidEncodedSize(uuidEncodedSize int) OpFunc {
	return func(option *Option) {
		option.sendPool.uuidEncoded = uuidEncodedSize
	}
}
